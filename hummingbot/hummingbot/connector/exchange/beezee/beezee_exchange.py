...
class BeezeeExchange(ExchangePyBase):
    ...
    def _allocate_trade_updates(self, orders: List[InFlightOrder], market: BeezeeMarket, history: List[Dict[str, Any]]) -> List[TradeUpdate]:
        ...
        for (side, display_price), bucket_orders in buckets.items():
            matching_trades = [
                ...
                if format(chain_price_to_display(trade["price"], market), "f") == display_price
                and (
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        candidate_ids = data_source.candidate_order_ids(open_refs, market.market_id, side)
        for candidate_id in candidate_ids:
            if candidate_id in claimed_exchange_ids:
                continue
            candidate_order = await data_source.get_market_order(market.market_id, side, candidate_id)
            if candidate_order is None:
                continue
            candidate_price = chain_price_to_display(candidate_order["price"], market)
            if candidate_price == tracked_order.price:
                state = OrderState.PARTIALLY_FILLED if tracked_order.executed_amount_base > Decimal("0") else OrderState.OPEN
                return OrderUpdate(
                    trading_pair=tracked_order.trading_pair,
                    update_timestamp=self.current_timestamp,
        ...
        if creation_tx_hash is not None and tx_code not in (None, 0):
            ...
            new_state = OrderState.CANCELED if cancel_tx_hash is not None else OrderState.PENDING_CANCEL
